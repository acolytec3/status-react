...
(defn save-mailserver-request-gaps [gaps]
...
(defn delete-mailserver-request-gaps [ids]
...
(defn delete-mailserver-request-gaps-by-chat-id [id]
  (json-rpc/call {:method "mailservers_deleteMailserverRequestGapsByChatID"
                  :params [id]
                  :on-success #(log/info "deleted gaps successfully")
                  :on-failure #(log/error "failed to delete gap" %)}))

(defn get-mailserver-request-gaps [chat-id success-fn]
...
(re-frame/reg-fx
 ::load-gaps
 (fn [[chat-id success-fn]]
   (get-mailserver-request-gaps chat-id success-fn)))

(re-frame/reg-fx
 ::save-gaps
 (fn [[gaps]]
   (save-mailserver-request-gaps gaps)))

(re-frame/reg-fx
 ::delete-gaps
 (fn [[ids]]
   (delete-mailserver-request-gaps ids)))

(re-frame/reg-fx
 ::delete-gaps-by-chat-id
 (fn [[id]]
   (delete-mailserver-request-gaps-by-chat-id id)))

(fx/defn load-gaps [cofx chat-id success-fn]
  {::load-gaps [chat-id success-fn]})
...
(fx/defn delete-gaps-by-chat-id [cofx chat-id]
  {::delete-gaps-by-chat-id [chat-id]})
